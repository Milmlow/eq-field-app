...
async function exportScheduleCSV() {
  ...
  // S1 Phase 2 — STATE.schedule is now scoped to a ±4 week window.
  // For an all-weeks export we have to fetch fresh (Q3 default).
  // _loadFullDataForExport returns a SNAPSHOT — it doesn't mutate
  // STATE, so the window stays bounded after export.
  ...
  // Build an indexed lookup over the snapshot, then reuse getPersonSchedule's
  // shape — we can't call getPersonSchedule directly since it reads STATE.
  const snapIndex = {};
